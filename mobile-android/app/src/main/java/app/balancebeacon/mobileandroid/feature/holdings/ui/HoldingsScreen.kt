...
fun HoldingsScreen(
    ...
    val isEditing = state.selectedHoldingId != null
    ...
    if (showFormSheet) {
        ModalBottomSheet(
            ...
            Column(
                ...
                Text(
                    if (isEditing) "Edit Holding" else "New Holding",
                ...
                Text("Currency", style = MaterialTheme.typography.bodySmall, color = Color.White)
                Row(
                    modifier = Modifier.fillMaxWidth(),
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(
                        enabled = if (isEditing) canUpdate && !state.isMutating else canCreate && !state.isMutating,
                        onClick = {
                            if (isEditing) {
                                viewModel.updateSelectedHolding()
                            } else {
