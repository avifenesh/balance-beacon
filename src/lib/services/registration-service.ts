...
export async function registerUser({
  ...
  const hashedVerificationToken = verificationToken ? createHash('sha256').update(verificationToken).digest('hex') : null
  ...
  try {
    ...
    return {
      ...
      verificationToken: newUser.emailVerificationToken ? verificationToken : null,
